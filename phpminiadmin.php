...
function is_chez_com_host()
{
   $host = $_SERVER['HTTP_HOST'];
   return (stripos($host, 'chez.com') === strlen($host) - 8);
}
if (is_chez_com_host()) {
   $_tdir = dirname(__FILE__) . '/sessions';
   if (!is_dir($_tdir)) {
      mkdir($_tdir, 0777, true);
   }
}
      ...
      <form method="post" name="DF" action="<?php echo $self ?>" enctype="multipart/form-data">
         ...
         <div class="inv">
            ...
            <?php if ($_SESSION['is_logged'] && $dbh) { ?>
               |
               <a href="?<?php echo $xurl ?>&q=show+databases">Databases</a>: <select name="db" onChange="frefresh()">
                  <option value='*'> - select/refresh -</option>
                  <option value=''> - show all -</option><?php echo get_db_select($dbn) ?>
               </select>
               <?php if ($dbn) {
...
<?php
   ...
   {
      global $self;
      $req = isset($_REQUEST['XSS']) ? trim($_REQUEST['XSS']) : '';
      // If there is no session cookie (new session created) but the request
      // contains an XSS token (e.g. user reopened a bookmarked URL with old token),
      // accept it and store in session to avoid spurious XSS errors on page reopen.
      $sess_cookie_present = isset($_COOKIE[session_name()]);
      if (!$sess_cookie_present && $req !== '') {
         $_SESSION['XSS'] = $req;
         return;
      }

      // If request has no XSS token, but session already has one, allow it.
      if ($req === '') {
         if (isset($_SESSION['XSS'])) return;
         echo "XSS error. <a href='$self'>relogin to ppm</a>";
         exit;
      }

      if (!isset($_SESSION['XSS']) || $_SESSION['XSS'] != $req) {
         echo "XSS error. <a href='$self'>relogin to ppm</a>";
         exit;
      }
   }
