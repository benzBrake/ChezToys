...
Modified by Ryan(https://doufu.ru)
					...
					elseif ($doing == 'extractfile') {
						if ($thefile) {
							...
							if (IS_WIN) {
								...
								// Unix/Linux系统检查是否是绝对路径
								if (substr($thefile, 0, 1) == '/') {
									$filepath = $thefile;
								} else {
									$filepath = $nowpath . $thefile;
								}
				...
				elseif ($action == 'editfile') {
					if (file_exists($opfile)) {
						...
						if (function_exists('mb_convert_encoding')) {
							$encoding = mb_detect_encoding($contents, array('UTF-8', 'GBK', 'GB2312', 'ASCII'));
							if ($encoding != 'UTF-8') {
					...
					echo '<script>
						...
						function initEditor() {
							...
							// 检查是否已经创建了 CodeMirror 实例
							if (textarea.nextSibling && textarea.nextSibling.className && textarea.nextSibling.className.indexOf("CodeMirror") !== -1) {
								return;
							}
							...
							// 根据文件扩展名设置语言模式
							var filename = "' . addslashes($opfile) . '";
							if (filename) {
								...
								if (modeMap[extension]) {
									...
									try {
										...
										console.log("无法加载模式: " + modeMap[extension] + ", 使用纯文本模式");
									}
								}
							}
							...
							setTimeout(function() {
								if (window.codeMirrorEditor) {
									...
									window.codeMirrorEditor.setSize(null, 500);
									
									// 确保原始 textarea 被隐藏
									var originalTextarea = document.getElementById("filecontent");
									if (originalTextarea) {
...
function realextract($file, $extractPath)
{
	...
	while (!gzeof($gz)) {
		...
		// 获取文件类型
		$type = octdec(substr($header, 156, 1));
		...
		if ($type == 0) { // 普通文件
			...
			// 跳过其他类型的内容
			gzseek($gz, gztell($gz) + $size);
		}

		// 按512字节对齐
		$padding = $size % 512;
		if ($padding > 0) {
			gzread($gz, 512 - $padding);
		}
	}
